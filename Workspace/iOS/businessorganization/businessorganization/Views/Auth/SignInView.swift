...
struct SignInView: View {
    ...
    @StateObject public var model: SignInModel = SignInModel()
            ...
            VStack {
                Image("Clipart-SignIn")
                        ...
                        .frame(width: 0.8192 * UIScreen.main.bounds.width,
                                height: 0.6584 * UIScreen.main.bounds.width,
                                alignment: .center)
                ...
                HStack {
                    VStack(alignment: .leading, spacing: 12) {
                        Text("Connection".localized())
                                .font(.title)
                                .fontWeight(.bold)
                        Text("Connection_Description".localized())
                                .opacity(0.5)
                    }
                    Spacer(minLength: 0)
                }
                ...
                Group {
                    ...
                    AuthSecureFieldView("Password".localized(), text: self.$model.password, autocapitalization: .none, disableAutocorrection: true, keyboardType: .default, textContentType: .password, imageSystemName: "lock", authButtonInformationFieldView: AuthButtonInformationFieldView({
                ...
                Button(action: { withAnimation {
                    self.selection = ForgotPasswordView.TAG
                } }) {
                    Text("Forgot_password_?".localized())
                            .font(.subheadline)
                ...
                Spacer(minLength: 0)
                HStack(spacing: 5) {
                    Text("Not_account_yet".localized())
                    Button(action: { withAnimation {
                        self.selection = SignUpView.TAG
                    } }) {
